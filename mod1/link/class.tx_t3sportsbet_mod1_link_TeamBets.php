...
class tx_t3sportsbet_mod1_link_TeamBets implements tx_cfcleague_mod1_Linker {
	...
	function makeLink($item, $formTool, $currentPid, $options) {
		...
		$out = $formTool->createEditButton('tx_t3sportsbet_teamquestions', $item->uid);
		$cnt = tx_t3sportsbet_util_serviceRegistry::getTeamBetService()->getBetCount($item);
		if($cnt) {
			$out .= '<br />'.$formTool->createSubmit('showTeamBets['.$item->uid.']', $GLOBALS['LANG']->getLL('label_showbets') . ' (' . $cnt. ')');
	
			$betset = $item->getBetSet();
			if(!$betset->isFinished())
				$out .= '<br />'.$formTool->createSubmit('resetTeamBets['.$item->uid.']', $GLOBALS['LANG']->getLL('label_resetbets'), $GLOBALS['LANG']->getLL('msg_resetbets'));
		}
